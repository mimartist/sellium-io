...
const GROUPS: InsightGroup[] = [
  {
    id: 'ads', title: 'Ad Recommendations', borderColor: '#5B5FC7', items: [
  ...
  {
    id: 'stock', title: 'Stock Recommendations', borderColor: '#EF4444', items: [
  ...
  {
    id: 'profitability', title: 'Profitability Recommendations', borderColor: '#10B981', items: [
  ...
  {
    id: 'general', title: 'General Recommendations', borderColor: '#F59E0B', items: [
...
const STATUS_MAP: Record<string, { l: string; bg: string; c: string }> = {
  new: { l: 'New', bg: '#EEF2FF', c: '#5B5FC7' },
  applied: { l: 'Applied', bg: '#ECFDF5', c: '#10B981' },
  skipped: { l: 'Skipped', bg: '#F8FAFC', c: '#94A3B8' },
}

const SUGGESTIONS = [
  'What are my top 5 most profitable products?',
  'Which campaigns should I pause?',
  'How much loss from out-of-stock products?',
  'What should I do to lower ACOS?',
  'How can I grow in the FR market?',
  'Which products are affected by the commission threshold?',
]
...
export default function AiOnerilerPage() {
  ...
  const [messages, setMessages] = useState<ChatMessage[]>([
    { role: 'assistant', content: 'Hello! I am your Sellometrix AI assistant. I can answer your questions about sales, ads, inventory, profitability, or strategy for your Mimosso account. What would you like to ask?' }
  ])
  ...
  const sendMessage = async (text?: string) => {
    ...
    try {
      ...
      setMessages(prev => [...prev, { role: 'assistant', content: data.reply || 'An error occurred.' }])
    } catch {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 12 }}>
        <div>
          <h1 style={{ fontSize: 24, fontWeight: 700, color: COLORS.text, margin: 0, display: 'flex', alignItems: 'center', gap: 10 }}>
            <GeminiLogo s={30} /> AI Recommendations & Assistant
          </h1>
          <p style={{ fontSize: 13, color: COLORS.sub, margin: '2px 0 0' }}>{totalInsights} recommendations · {newCount} new · Mimosso account</p>
        </div>
      </div>
        ...
        <div className="ai-chat-inner">
          ...
          <div style={{ position: 'relative', zIndex: 1 }}>
            ...
            <div style={{ padding: '20px 24px 14px', display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <div>
                <div style={{ fontSize: 17, fontWeight: 700, color: COLORS.text }}>Sellometrix AI</div>
                <div style={{ fontSize: 11, color: COLORS.sub }}>Smart assistant with database access</div>
              </div>
              <div style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 600, padding: '4px 14px', borderRadius: 20, background: 'rgba(16,185,129,.1)', color: COLORS.green }}>● Active</div>
            </div>
                ...
                <div key={i} style={{ display: 'flex', flexDirection: 'column', alignItems: m.role === 'user' ? 'flex-end' : 'flex-start' }}>
                  {m.role === 'assistant' && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                      ...
                      <span style={{ fontSize: 10, fontWeight: 600, color: COLORS.sub }}>Sellometrix AI</span>
                    </div>
                  )}
            ...
            <div style={{ padding: '16px 24px 20px', display: 'flex', gap: 10, alignItems: 'center' }}>
              <input
                ...
                placeholder="Ask about your Mimosso account..."
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 20, flexWrap: 'wrap' }}>
        {[{ id: 'all', l: `All (${totalInsights})` }, ...GROUPS.map(g => ({ id: g.id, l: `${g.title.replace(' Recommendations', '')} (${g.items.length})` }))].map(f => (
      ...
      {filteredGroups.map((group, gi) => (
        <div key={gi} style={{ ...CARD_STYLE, marginBottom: 20, overflow: 'hidden', borderTop: `1px solid ${group.borderColor}30`, borderRight: `1px solid ${group.borderColor}30`, borderBottom: `1px solid ${group.borderColor}30`, borderLeft: `2px solid ${group.borderColor}`, boxShadow: `0 2px 12px ${group.borderColor}08` }}>
          ...
          <div style={{ padding: '18px 24px 14px', borderBottom: '1px solid #F1F5F9' }}>
            <div style={{ fontSize: 18, fontWeight: 700, color: COLORS.text }}>{group.title}</div>
            <div style={{ fontSize: 12, color: COLORS.sub, marginTop: 2 }}>{group.items.length} recommendations · {group.items.filter(i => i.status === 'new').length} new</div>
          </div>
                ...
                <div key={ii} style={{ padding: '14px 24px', borderBottom: ii < group.items.length - 1 ? '1px solid #F8FAFC' : 'none', display: 'flex', gap: 16, alignItems: 'flex-start' }}>
                  ...
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                      ...
                      <span style={{ marginLeft: 'auto', fontSize: 10, fontWeight: 600, padding: '2px 10px', borderRadius: 12, background: st.bg, color: st.c, flexShrink: 0 }}>{st.l}</span>
                    </div>
                    ...
                    {item.status === 'new' && (
                      <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
                        <button style={{ padding: '6px 16px', borderRadius: 6, border: 'none', background: COLORS.accent, color: '#fff', fontSize: 11, fontWeight: 600, cursor: 'pointer' }}>Apply</button>
                        <button style={{ padding: '6px 16px', borderRadius: 6, border: '1px solid #E2E8F0', background: '#fff', color: '#64748B', fontSize: 11, fontWeight: 500, cursor: 'pointer' }}>Skip</button>
                      </div>
                    )}
                  </div>
                </div>
